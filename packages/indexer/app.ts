...
async function setMovieSubtitlesToDatabase({
	...
	try {
		...
		const subtitlesFolderAbsolutePath = path.join(
			__dirname,
			"..",
			"indexer",
			"subtitles",
		...
		await supabase.storage
			.from("subtitles")
			.upload(subtitleSrtFileName, srtFileToUploadBuffer, {
		...
		const successSoundPath = path.join(
			__dirname,
			"..",
			"indexer",
			"success_short_high.wav",
		);
...
async function getSubtitlesFromMovie(
	...
	if (!torrentsWithoutCineRecordings.length) {
		console.log(
			`4.${index}) No se encontraron torrents para la película "${title}" \n`,
		);
		return;
	}

	console.log(
		`4.${index}) Torrents encontrados para la pelicula "${title}" \n`,
	);
	...
	for await (const [index, torrentData] of Object.entries(
		torrentsWithoutCineRecordings,
	)) {
		console.log(
			`4.${index}) Procesando torrent`,
			`"${torrentData.title}"`,
			"\n",
		);
		...
		// 6. Find video file
		const filesSortedByLength = files.toSorted(
			(fileA, fileB) => fileB.length - fileA.length,
		);
		...
		if (!movieData) {
			console.log(
				`4.${index}) No se encontró metadata para la película "${title}" \n`,
			);
			continue;
		}
		...
		const enabledSubtitleProviders = getEnabledSubtitleProviders(
			subtitleGroups,
			["SubDivX"],
		);

		for await (const [
			indexSubtitleProvider,
			enabledSubtitleProvider,
		] of Object.entries(enabledSubtitleProviders)) {
			const { getSubtitleFromProvider, id, name } = enabledSubtitleProvider;

			try {
				console.log(
					`4.${index}.${indexSubtitleProvider}) Buscando subtítulo en ${name}`,
				);
				...
				console.log(
					`4.${index}.${indexSubtitleProvider}) Subtítulo encontrado para ${subtitleGroup}`,
				);
				...
				console.log(
					`4.${index}.${indexSubtitleProvider}) Subtítulo no encontrado en ${name} \n`,
				);
			}
		}
...
async function mainIndexer(
	...
	try {
		...
		const { totalPages, totalResults } = await getTmdbMoviesTotalPagesArray(
			moviesYear,
			isDebugging,
		);
		console.log(
		...
		for await (const tmbdMoviesPage of totalPages) {
			...
			for await (const [index, movie] of Object.entries(movies)) {
				...
				try {
					...
					console.error(
						"Ningún subtítulo encontrado para la película",
						movie.title,
					);
				}
			}
