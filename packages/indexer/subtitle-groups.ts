...
export async function saveSubtitleGroupsToDb(
	...
	for (const subtitleGroupKey in SUBTITLE_GROUPS) {
		...
		const subtitleGroupExists = data?.find(
			(subtitleGrouInDb) => subtitleGrouInDb.name === subtitleGroup.name,
		);
...
export function getEnabledSubtitleProviders(
	subtitleGroups: SubtitleGroupMap,
	providers: SubtitleGroupNames[],
) {
	return Object.values(subtitleGroups)
		.map((subtitleGroup) => {
			const subtitleGroupGetter =
				SUBTITLE_GROUPS_GETTERS[subtitleGroup.name as SubtitleGroupNames];
