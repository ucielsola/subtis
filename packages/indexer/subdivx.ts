...
export async function getSubtitlesFromSubDivXForTitle({
  ...
  try {
    ...
    const IMDB_ID_BLACKLIST_FOR_SEARCH_BY_QUERY = [] as string[];
    const isWhitelistedForSearchByQuery = !IMDB_ID_BLACKLIST_FOR_SEARCH_BY_QUERY.includes(fullImdbId);

    if (isWhitelistedForSearchByQuery) {
