...
function Marquee({ children, repeat = 2, baseVelocity = 1, inverted = false, className, ...props }: Props) {
  ...
  useAnimationFrame((_: number, delta: number) => {
    ...
    const velocityFactor = 1 + Math.abs(smoothVelocity.get() * 0.05);
  ...
  return (
    <div ref={setIntersectionRef} className={cn("flex overflow-hidden relative", className)} {...props}>
      <motion.div ref={containerRef} className="flex whitespace-nowrap" style={{ x: baseX }}>
        {new Array(repeat).fill(null).map((_, i) => (
          <div key={i} className="flex shrink-0" aria-hidden={i !== 0} data-nosnippet={i !== 0 ? "" : undefined}>
            {children}
          </div>
        ))}
      </motion.div>
